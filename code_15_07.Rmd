...
simulate_10_iterations <- function() {
  results <- replicate(10, simulate_mpox_impact(M, CFR, params, vacc_coverage = 0.7, ve_i = 0.8, ve_s = 0.5, ve_d = 0.9), simplify = FALSE)
  ...
  results_df <- bind_rows(lapply(results, function(x) x[!names(x) %in% c("severity_counts_without_vaccination", "severity_counts_with_vaccination")]))
...
results_replication_1000_times <- replicate(1000, simulate_10_iterations(), simplify = FALSE) %>%
  bind_rows()
